...
namespace ShaderInfo {
    std::map<std::string, ShaderType> extensions = {
            ...
            {".tes.glsl",  ShaderType::TESS_EVALUATION},
...
void Shader::detachAndDeleteShaderObjects() {
    ...
    glGetAttachedShaders(handle, numShaders, nullptr, shaders.data());
...
void Shader::compileShader(const char *fileName) {
    ...
    ShaderType type;
    auto it = ShaderInfo::extensions.find(ext);
    if (it != ShaderInfo::extensions.end()) {
        type = it->second;
    } else {
...
string Shader::getExtension(const char *fileName) {
    string nameStr(fileName);

    size_t dotLoc = nameStr.find_last_of('.');
    if (dotLoc != string::npos) {
        string ext = nameStr.substr(dotLoc);
        if (ext == ".glsl") {
...
void Shader::compileShader(const char *fileName, ShaderType type) {
    ...
    if (!inFile.is_open()) {
        throw ShaderException("Unable to open: " + string(fileName));
    }
...
void Shader::compileShader(const std::string &source, ShaderType type, const char *fileName) {
    ...
    GLuint shaderHandle = glCreateShader(static_cast<std::underlying_type_t<ShaderType>>(type));
...
void Shader::link() {
    if (linked) return;
    if (handle <= 0) throw ShaderException("Program has not been compiled.");
