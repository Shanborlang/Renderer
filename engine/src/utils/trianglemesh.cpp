...
void
TriangleMesh::initBuffers(std::vector<GLuint> *indices, std::vector<GLfloat> *points, std::vector<GLfloat> *normals,
                          std::vector<GLfloat> *texCoords, std::vector<GLfloat> *tangents) {
    ...
    glBufferData(GL_ARRAY_BUFFER, long(points->size() * sizeof(GLfloat)), points->data(), GL_STATIC_DRAW);

    glGenBuffers(1, &normBuf);
    buffers.push_back(normBuf);
    glBindBuffer(GL_ARRAY_BUFFER, normBuf);
    glBufferData(GL_ARRAY_BUFFER, long(normals->size() * sizeof(GLfloat)), normals->data(), GL_STATIC_DRAW);

    if( texCoords != nullptr ) {
        glGenBuffers(1, &tcBuf);
        buffers.push_back(tcBuf);
        glBindBuffer(GL_ARRAY_BUFFER, tcBuf);
        glBufferData(GL_ARRAY_BUFFER, long(texCoords->size() * sizeof(GLfloat)), texCoords->data(), GL_STATIC_DRAW);
    }
    ...
    if( tangents != nullptr ) {
        ...
        glBufferData(GL_ARRAY_BUFFER, long(tangents->size() * sizeof(GLfloat)), tangents->data(), GL_STATIC_DRAW);
    }
...
void TriangleMesh::render() const {
    ...
    glBindVertexArray(vao);
    glDrawElements(GL_TRIANGLES, (int)nVerts, GL_UNSIGNED_INT, nullptr);
    glBindVertexArray(0);
}
