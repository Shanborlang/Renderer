...
Torus::Torus(GLfloat outerRadius, GLfloat innerRadius, GLuint nsides, GLuint nrings) {
    ...
    const float ringFactor = glm::two_pi<float>() / (float)nrings;
    const float sideFactor = glm::two_pi<float>() / (float)nsides;
    ...
    for( GLuint ring = 0; ring <= nrings; ring++ ) {
        ...
        for( GLuint side = 0; side < nsides; side++ ) {
            ...
            tidx += 2;
            // Normalize
            float len = glm::sqrt( n[idx] * n[idx] +
                              n[idx+1] * n[idx+1] +
                              n[idx+2] * n[idx+2] );
            n[idx] /= len;
            n[idx+1] /= len;
    ...
    for( GLuint ring = 0; ring < nrings; ring++ ) {
        ...
        for( GLuint side = 0; side < nsides; side++ ) {
            int nextSide = int((side+1) % nsides);
            // The quad
